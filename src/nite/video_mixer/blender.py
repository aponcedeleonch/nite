...
def get_video_2_weighted(video_2: np.ndarray, alpha: np.ndarray) -> np.ndarray:
    return (255.0 * video_2 * (alpha / 255.0)).astype(np.uint8)


def blend_normal(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return video_2_weighted


def blend_darken(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return np.where(video_1 < video_2_weighted, video_1, video_2_weighted)


def blend_lighten(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return np.where(video_1 > video_2_weighted, video_1, video_2_weighted)


def blend_multiply(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return (video_1 * video_2_weighted / 255.0).astype(np.uint8)


def blend_screen(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return (255.0 * ((1 - video_1) * (1 - video_2_weighted))).astype(np.uint8)


def blend_add(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return np.clip(video_1 + video_2_weighted, 0, 255, dtype=np.uint8)


def blend_difference(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    video_2_weighted = video_2 if alpha is None else get_video_2_weighted(video_2, alpha)
    return np.abs(video_1 - video_2_weighted, dtype=np.uint8)


def blend_pick(video_1: np.ndarray, video_2: np.ndarray, alpha: Optional[np.ndarray]) -> np.ndarray:
    return video_2
...
class BlenderMath(Blender):
    ...
    def blend(self, frames: List[cv2.typing.MatLike]) -> cv2.typing.MatLike:
        return self.blend_function(*frames)
...
class BlendWithSong:
    ...
    def blend(self, frames: List[cv2.typing.MatLike], should_blend: bool) -> cv2.typing.MatLike:
        if should_blend:
