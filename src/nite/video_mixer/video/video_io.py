...
from nite.video_mixer.video.video import VideoFramesPath, VideoMetadata, VideoFramesImg, VideoFrames
...
class VideoReader:
    ...
    async def from_video(self, input_video: str) -> VideoFramesImg:
        ...
        frame_count = 0
        frames_imgs = []
        logger.info(f'Converting video {metadata.name} to frames. Number of frames: {metadata.num_frames}')
    ...
    async def from_frames(self, input_frames_dir: str, width: int, height: int, is_alpha: bool = False) -> VideoFramesPath:
        ...
        if image_frames_dir_resol.is_dir():
            metadata = self._read_metadata_from_json(str(image_frames_dir_resol))
            video_frames_paths = VideoFramesPath(metadata=metadata, image_frames_dir=image_frames_dir_resol)
